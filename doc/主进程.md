...
| plugins:list | renderer→main | - | PluginListEntry[] | 已安装插件列表（当前内置见 `doc/插件市场.md`） |
| plugins:setEnabled | renderer→main | pluginId, enabled | boolean | 启用/禁用插件；成功后向窗口发送 `plugins:preferencesChanged` |
| plugins:exportConversation | - | - | - | **未注册**：`PluginRegistry.exportConversationMarkdown` 存在但无 IPC / preload |
| pavatar:listPacks | renderer→main | - | PAvatarPackResolved[] | 扫描 `userData/pavatar-packs` |
| pavatar:getActivePack | renderer→main | - | PAvatarPackResolved \| null | 当前活跃形象包 |
| pavatar:setActivePack | renderer→main | packId, version | boolean | 写入插件 config 并发送 `pavatar:activePackChanged` |
...
**主进程 → 渲染（`webContents.send`）**：`plugins:preferencesChanged`、`pavatar:activePackChanged`、`plugin:dispatch`（内置插件向 UI 派发消息，如像素小人情绪）。
